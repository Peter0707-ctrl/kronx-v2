...
import { expandImagePrompt } from './imagePromptEnhancer'
...
export function matchImageGenerationRequest(query: string): { isImageGen: boolean; prompt: string; markdown: string } {
  ...
  // Exclude programming or non-visual text generation requests
  if (/\b(write code|generate code|create code|python code|react component|html page|css code|sql query|essay|insha)\b/i.test(lower)) {
    return { isImageGen: false, prompt: '', markdown: '' }
  }

  const isGen =
    /^(generate|create|draw|make|design|render|produce|tengeneza|chora|leta|niletee)\s+(an?\s+)?(image|picture|photo|illustration|art|drawing|painting|picha|mchoro)\s+(of|for|about|ya|za|showing|depicting)?/i.test(
      lower
    ) ||
    /^(tengeneza|chora)\s+picha\s+(ya|za|kwa ajili ya)?/i.test(lower) ||
    /^(chora|paint|sketch|illustrate)\s+(an?\s+)?([a-zA-Z0-9\s]+)/i.test(lower) ||
    /^(draw|paint)\s+(an?\s+)?(image|picture|photo|mchoro)/i.test(lower) ||
    /^(generate|create)\s+([a-zA-Z0-9\s]+)\s+(picture|image|photo|drawing|illustration)/i.test(lower)

  if (!isGen) return { isImageGen: false, prompt: '', markdown: '' }

  const expanded = expandImagePrompt(clean)
