...
const MessageBubble = memo(function MessageBubble({ message, isStreaming, onRegenerate, onEditAndResend }: Props) {
  ...
  const handleShareWhatsApp = () => {
    const cleanText = message.content.replace(/\[IMAGE:[\s\S]*?\]/gi, '').slice(0, 1500)
    const text = `*Copetra AI Answer:*\n\n${cleanText}\n\n_Generated by Copetra AI: https://miraculous-forgiveness-production-10d4.up.railway.app_`
        ...
        {isAi && message.content && !isStreaming && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '12px', position: 'relative' }}>
            ...
            <button
              ...
              style={{
                background: isSpeaking ? 'rgba(2, 132, 199, 0.15)' : 'none',
                border: isSpeaking ? '1px solid #0284c7' : 'none',
                borderRadius: '6px',
            ...
            >
              <svg width={16} height={16} viewBox="0 0 24 24" fill={isSpeaking ? '#0284c7' : 'none'} stroke="currentColor" strokeWidth={2}>
            ...
            <button
              ...
              style={{ background: 'none', border: 'none', color: '#16a34a', cursor: 'pointer', padding: '4px', display: 'flex', alignItems: 'center' }}
            >
              <svg width={16} height={16} viewBox="0 0 24 24" fill="currentColor">
                <path d="M12.031 6.172c-3.181 0-5.767 2.586-5.768 5.766-.001 1.298.38 2.27 1.019 3.287l-.711 2.598 2.664-.698c.969.586 1.776.815 2.796.815 3.182 0 5.767-2.587 5.768-5.766 0-3.18-2.586-5.766-5.768-5.766zm9.969 5.766c0 5.522-4.477 10-10 10-1.758 0-3.411-.453-4.85-1.246l-5.15 1.352 1.378-5.029c-.879-1.488-1.378-3.218-1.378-5.077 0-5.522 4.478-10 10-10s10 4.478 10 10z" />
              </svg>
            </button>
            ...
            <button
              ...
              style={{ background: 'none', border: 'none', color: '#dc2626', cursor: 'pointer', padding: '4px', display: 'flex', alignItems: 'center' }}
            >
              <svg width={16} height={16} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
